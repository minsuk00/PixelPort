# Vendored verbatim from anatomix (https://github.com/neel-dey/anatomix),
# MIT License, Copyright 2024 Neel Dey. Bundled unchanged so this inference
# script depends only on torch (not the full anatomix package and its
# deps). Only the `Unet` class is used.
#
# Upstream header:
        ...
        if len(layers) > 0:
            ...
            for layer_id, layer in enumerate(self.model):
                ...
                if layer_id in layers:
                    ...
                    if verbose:
                        print(
                            "%d: skipping %s" % (layer_id, layer.__class__.__name__),
                            feat.size(),
                        )
                    pass
